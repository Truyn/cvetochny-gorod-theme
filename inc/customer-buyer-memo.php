<?php
/**
 * Customer care memo for flower orders.
 *
 * Adds a printable care memo, customer-email copy and a compact order-details
 * card. The memo intentionally focuses on inspection at handover and flower
 * care after receipt. Return terms are maintained separately by the store.
 *
...
const CG_BUYER_MEMO_VERSION = '12.08.2026';

/** Public contact data shown in the compact memo. */
function cg_buyer_memo_contacts() {
    $legal = function_exists('cg_legal_get_settings') ? cg_legal_get_settings() : [];
    $phone = !empty($legal['phone'])
        ? (string) $legal['phone']
        : get_theme_mod('cg_phone', '+7 (930) 411-98-55');
    ...
    <div class="cg-buyer-memo cg-buyer-memo--<?php echo esc_attr($context); ?>">
        <div class="cg-buyer-memo__head">
            <div>
                ...
                <h3>Получили букет — проверьте его и правильно ухаживайте</h3>
            </div>
            <strong>Редакция <?php echo esc_html(CG_BUYER_MEMO_VERSION); ?></strong>
        </div>

        <div class="cg-buyer-memo__notice">
            <b>Осмотрите букет при получении.</b>
            Проверьте свежесть, состав, количество цветов, целостность стеблей и упаковки, а также соответствие согласованному заказу. Если всё в порядке, букет считается принятым без замечаний по его видимому состоянию на момент вручения.
        </div>
        ...
        <div class="cg-buyer-memo__grid">
            <section>
                ...
                <ol>
                    <li><b>Вода.</b> Поставьте букет в чистую прохладную воду как можно скорее. Флористическую губку в композиции регулярно поддерживайте влажной.</li>
                    <li><b>Чистота.</b> Используйте чистую вазу, регулярно меняйте воду и убирайте листья, оказавшиеся ниже уровня воды.</li>
                    <li><b>Срез.</b> Для букета в вазе обновляйте срез стеблей чистым острым инструментом, если это подходит конкретным цветам.</li>
                    <li><b>Прохлада.</b> Не ставьте цветы под прямое солнце, возле батареи, обогревателя, кондиционера, на сильный сквозняк или мороз.</li>
            ...
            <section>
                <h4>После получения</h4>
                <ul>
                    <li>Срезанные цветы — живой скоропортящийся товар, состояние которого после вручения напрямую зависит от воды, температуры, освещения и ухода.</li>
                    <li>Не оставляйте букет без воды, на солнце, возле батареи или обогревателя, на морозе, в перегретом автомобиле или под сильным потоком кондиционера.</li>
                    <li>Увядание, пересыхание, перегрев, переохлаждение или механические повреждения, возникшие после вручения из-за условий хранения, перевозки получателем или ухода, относятся к условиям содержания букета после получения.</li>
                    <li>Магазин может фотографировать готовый букет перед отправкой, чтобы зафиксировать его внешний вид и состояние перед доставкой.</li>
                </ul>
            </section>
        </div>

        <div class="cg-buyer-memo__claim">
            <h4>Если проблема заметна при вручении</h4>
            <p>Сообщите об этом магазину сразу при получении или как можно быстрее после обнаружения. По возможности сделайте фотографии букета целиком и проблемного места — это поможет сопоставить состояние заказа с фотографией перед отправкой.</p>
            <p><b>Телефон:</b> <?php echo esc_html($contacts['phone']); ?> · <b>Режим работы:</b> <?php echo esc_html($contacts['hours']); ?></p>
        </div>
...
function cg_buyer_memo_plain_text() {
    ...
    $lines = [
        ...
        '',
        'ПРИ ПОЛУЧЕНИИ: осмотрите свежесть, состав, количество цветов, целостность стеблей и упаковки, соответствие согласованному заказу. Если всё в порядке, букет принят без замечаний по его видимому состоянию на момент вручения.',
        '',
        'КАК УХАЖИВАТЬ:',
        '1. Как можно скорее поставьте букет в чистую прохладную воду; флористическую губку поддерживайте влажной.',
        '2. Используйте чистую вазу, регулярно меняйте воду и не оставляйте листья ниже уровня воды.',
        ...
        '',
        'ПОСЛЕ ПОЛУЧЕНИЯ:',
        '- Состояние срезанных цветов после вручения напрямую зависит от воды, температуры, освещения и ухода.',
        '- Увядание, пересыхание, перегрев, переохлаждение и механические повреждения после вручения относятся к условиям хранения, перевозки получателем и ухода после получения.',
        '- Магазин может фотографировать готовый букет перед отправкой для фиксации его состояния.',
        '',
        'ЕСЛИ ПРОБЛЕМА ЗАМЕТНА ПРИ ВРУЧЕНИИ: сообщите магазину сразу или как можно быстрее и по возможности сделайте фотографии.',
        'Телефон: ' . $contacts['phone'],
        'Режим работы: ' . $contacts['hours'],
        'Печатная памятка: ' . cg_buyer_memo_print_url(),
        '',
    ];
...
/** Add the care memo to the main customer order emails. */
function cg_buyer_memo_email($order, $sent_to_admin, $plain_text, $email) {
    ...
    echo '<p style="margin:0 0 12px;line-height:1.65;"><strong>При получении осмотрите букет.</strong> Проверьте свежесть, состав и отсутствие видимых повреждений. Если всё в порядке, дальше важно соблюдать условия ухода.</p>';
    ...
    echo '<li>не оставляйте букет надолго в душной машине или другом перегретом месте.</li>';
    echo '</ul>';
    echo '<p style="margin:0 0 8px;line-height:1.6;">Если проблема видна при вручении, сообщите нам как можно быстрее и по возможности сделайте фотографии.</p>';
    echo '<p style="margin:0;line-height:1.6;"><strong>Телефон:</strong> ' . esc_html($contacts['phone']) . ' · <strong>Режим работы:</strong> ' . esc_html($contacts['hours']) . '<br><a href="' . esc_url(cg_buyer_memo_print_url()) . '">Открыть полную памятку по уходу</a></p>';
    echo '</div>';
}
...
/** Display the active memo revision in the order admin. */
...
function cg_buyer_memo_admin_page() {
    ...
    <div class="wrap">
        ...
        <p style="max-width:900px;font-size:14px;line-height:1.65">Памятка автоматически добавляется в клиентские письма WooCommerce и отображается в деталях заказа. Её можно распечатать и передавать вместе с букетом как короткую инструкцию по проверке при получении и дальнейшему уходу.</p>
        ...
        <div style="max-width:980px;margin-top:22px;padding:18px;border:1px solid #ddd;background:#fff;border-radius:12px">
            ...
            <ul style="list-style:disc;padding-left:20px;line-height:1.7">
                <li>просьбу проверить состояние букета при вручении;</li>
                <li>пять основных правил ухода;</li>
                <li>предупреждение о солнце, батареях, жаре, морозе и отсутствии воды;</li>
                <li>информацию о фотофиксации букета перед отправкой;</li>
                <li>телефон магазина и режим работы.</li>
            </ul>
        </div>
    </div>
    <?php
...
function cg_buyer_memo_print_view() {
    ...
    <body>
        ...
        <main class="cg-print-sheet">
            ...
            <div class="cg-print-note">Передайте памятку вместе с букетом, чтобы получатель сразу видел основные правила ухода.</div>
        </main>
    </body>
    </html>
    <?php
